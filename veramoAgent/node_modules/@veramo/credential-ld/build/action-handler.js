...
export class CredentialIssuerLD {
    ...
    async createVerifiablePresentationLD(args, context) {
        ...
        try {
            const { signingKey, verificationMethodId } = await this.findSigningKeyWithId(context, identifier, args.keyRef, args.resolutionOptions);
            let { now } = args;
    ...
    async verifyPresentationLD(args, context) {
        ...
        return this.ldCredentialModule.verifyPresentation(presentation, args.challenge, args.domain, { ...args, now }, context);
    }
    async findSigningKeyWithId(context, identifier, keyRef, resolutionOptions) {
        const extendedKeys = await mapIdentifierKeysToDoc(identifier, 'assertionMethod', context, resolutionOptions);
