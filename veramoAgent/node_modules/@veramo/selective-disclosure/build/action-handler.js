...
import { asArray, bytesToBase64, computeEntryHash, decodeCredentialToObject, extractIssuer, intersect, } from '@veramo/utils';
/**
...
export class SelectiveDisclosure {
    methods;
    schema = schema.ISelectiveDisclosure;
    constructor() {
        this.methods = {
     ...
     * @beta This API may change without a BREAKING CHANGE notice.
     */
    async createSelectiveDisclosureRequest(args, context) {
        try {
            ...
            const algs = ['ES256K', 'ES256K-R', 'EdDSA', 'ES256'];
            const key = identifier.keys.find((k) => {
                return (Object.keys(KEY_ALG_MAPPING).includes(k.type) &&
     ...
     * @beta This API may change without a BREAKING CHANGE notice.
     */
    async getVerifiableCredentialsForSdr(args, context) {
        const result = [];
     ...
     * @beta This API may change without a BREAKING CHANGE notice.
     */
    async validatePresentationAgainstSdr(args, context) {
        let valid = true;
        let claims = [];
        for (const credentialRequest of args.sdr.claims) {
     ...
     * @beta This API may change without a BREAKING CHANGE notice.
     */
    async createProfilePresentation(args, context) {
