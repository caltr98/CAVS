...
exports.Ed25519VerificationKey2020 = void 0;
const baseX_1 = require("./baseX");
const ed25519_1 = require("./ed25519");
const keypair_1 = require("@digitalcredentials/keypair");
...
class Ed25519VerificationKey2020 extends keypair_1.KeyPair {
    /**
     ...
     */
    constructor({ id, controller, revoked, publicKeyMultibase, privateKeyMultibase } = {}) {
    ...
    static fromEd25519VerificationKey2018({ keyPair }) {
        const publicKeyMultibase = _encodeMbKey(MULTICODEC_ED25519_PUB_HEADER, baseX_1.base58btc.decode(keyPair.publicKeyBase58));
        ...
        if (keyPair.privateKeyBase58) {
            keyPair2020.privateKeyMultibase = _encodeMbKey(MULTICODEC_ED25519_PRIV_HEADER, baseX_1.base58btc.decode(keyPair.privateKeyBase58));
        }
        return keyPair2020;
    }
    /**
     ...
     */
    static async fromJsonWebKey2020({ id, type, controller, publicKeyJwk, privateKeyJwk }) {
        ...
        const publicKeyBytes = baseX_1.base64url.decode(publicKeyBase64Url);
        const publicKeyMultibase = _encodeMbKey(MULTICODEC_ED25519_PUB_HEADER, publicKeyBytes);
        const inputKeyDocument = {
            id,
            controller,
            publicKeyMultibase
        };
        if (privateKeyJwk) {
            ...
            const privateKeyBytes = baseX_1.base64url.decode(privateKeyBase64Url);
            // Concat the private and public key bytes
            const combinedPrivatePublicBytes = new Uint8Array(privateKeyBytes.length + publicKeyBytes.length);
            combinedPrivatePublicBytes.set(privateKeyBytes);
            combinedPrivatePublicBytes.set(publicKeyBytes, privateKeyBytes.length);
            inputKeyDocument.privateKeyMultibase = _encodeMbKey(MULTICODEC_ED25519_PRIV_HEADER, combinedPrivatePublicBytes);
        }
    ...
    static async generate({ seed, ...keyPairOptions } = {}) {
        let keyObject;
        if (seed) {
            keyObject = await ed25519_1.default.generateKeyPairFromSeed(seed);
        }
        else {
            keyObject = await ed25519_1.default.generateKeyPair();
        }
    ...
    get _publicKeyBuffer() {
        ...
        const publicKeyMulticodec = baseX_1.base58btc.decode(this.publicKeyMultibase.substr(1));
    ...
    get _privateKeyBuffer() {
        ...
        // remove multibase header
        const privateKeyMulticodec = baseX_1.base58btc.decode(this.privateKeyMultibase.substr(1));
        // remove multicodec header
     ...
     *   (to JSON, etc), for use in DIDs, Linked Data Proofs, etc.
     */
    toEd255519VerificationKey2018({ publicKey = false, privateKey = false, includeContext = false } = {}) {
        if (!(publicKey || privateKey)) {
            throw new TypeError('Export requires specifying either "publicKey" or "privateKey".');
        }
        const exportedKey = {
        ...
        if (publicKey && this._publicKeyBuffer) {
            exportedKey.publicKeyBase58 = baseX_1.base58btc.encode(this._publicKeyBuffer);
        }
        if (privateKey && this._privateKeyBuffer) {
            exportedKey.privateKeyBase58 = baseX_1.base58btc.encode(this._privateKeyBuffer);
        }
        if (this.revoked) {
            exportedKey.revoked = this.revoked;
        }
    ...
    toJwk({ publicKey = true, privateKey = false } = {}) {
        ...
        if (publicKey && this._publicKeyBuffer) {
            jwk.x = baseX_1.base64url.encode(this._publicKeyBuffer);
        }
        if (privateKey && this._privateKeyBuffer) {
            // the private key buffer is a concatenation of <priv key bytes><pub key bytes>
            // however, the JWK wants just the private key
            jwk.d = baseX_1.base64url.encode(this._privateKeyBuffer.slice(0, this._privateKeyBuffer.length - this._publicKeyBuffer.length));
        }
        return jwk;
    }
    /**
    ...
    async jwkThumbprint() {
        ...
        const publicKey = baseX_1.base64url.encode(this._publicKeyBuffer);
        const serialized = `{"crv":"Ed25519","kty":"OKP","x":"${publicKey}"}`;
        const data = new TextEncoder().encode(serialized);
        return baseX_1.base64url.encode(new Uint8Array(await ed25519_1.default.sha256digest(data)));
    }
    /**
    ...
    verifyFingerprint({ fingerprint }) {
        ...
        try {
            fingerprintBuffer = baseX_1.base58btc.decode(fingerprint.substr(1));
        ...
        const verified = fingerprintBuffer[0] === MULTICODEC_ED25519_PUB_HEADER[0] &&
            fingerprintBuffer[1] === MULTICODEC_ED25519_PUB_HEADER[1] &&
            buffersEqual;
        if (!verified) {
            return {
                error: new Error('Invalid fingerprint encoding (expecting 0xed01 byte prefix).'),
                verified: false
            };
        }
        return { verified };
    ...
    signer() {
        ...
        return {
            async sign({ data }) {
                ...
                return ed25519_1.default.sign(privateKeyBuffer, data);
            },
            id: this.id
        };
    }
    verifier() {
        ...
        return {
            async verify({ data, signature }) {
                ...
                return ed25519_1.default.verify(publicKeyBuffer, data, signature);
            },
            id: this.id
        };
    }
...
function _isValidKeyHeader(multibaseKey, expectedHeader) {
    ...
    const keyBytes = baseX_1.base58btc.decode(multibaseKey.slice(1));
...
function _encodeMbKey(header, key) {
    ...
    return MULTIBASE_BASE58BTC_HEADER + baseX_1.base58btc.encode(mbKey);
}
