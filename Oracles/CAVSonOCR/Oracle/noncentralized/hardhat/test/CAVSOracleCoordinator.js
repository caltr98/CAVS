...
describe("CAVSOracleCoordinator", function () {
  ...
  it("registers an explicit oracle slot and exposes its data", async function () {
    ...
    const endpoint = "http://127.0.0.1:21002/requests";

    await expect(contract.connect(oracle).registerOracle(2n, did, endpoint))
      .to.emit(contract, "OracleRegistered")
      .withArgs(2n, oracle.address, did, endpoint);
    ...
    expect(registered.endpoint).to.equal(endpoint);
  ...
  it("updates the same account's assigned slot without allocating a new slot", async function () {
    ...
    await contract.connect(oracle).registerOracle(1n, "did:ethr:sepolia:0xabc", "http://oracle-one");
    await expect(contract.connect(oracle).registerOracle(1n, "did:ethr:sepolia:0xdef", "http://oracle-two"))
      .to.emit(contract, "OracleRegistered")
      .withArgs(1n, oracle.address, "did:ethr:sepolia:0xdef", "http://oracle-two");
    ...
    expect(registered.endpoint).to.equal("http://oracle-two");
  ...
  it("registers multiple oracle slots in the configured network", async function () {
    ...
    await expect(contract.connect(oracle).registerOracle(0n, "did:ethr:sepolia:0x123", "http://oracle-zero"))
      .to.emit(contract, "OracleRegistered")
      .withArgs(0n, oracle.address, "did:ethr:sepolia:0x123", "http://oracle-zero");
    await expect(contract.connect(otherOracle).registerOracle(3n, "did:ethr:sepolia:0x456", "http://oracle-three"))
      .to.emit(contract, "OracleRegistered")
      .withArgs(3n, otherOracle.address, "did:ethr:sepolia:0x456", "http://oracle-three");
  ...
  it("rejects oracle IDs outside the configured network", async function () {
    ...
    await expect(contract.connect(oracle).registerOracle(2n, "did:ethr:sepolia:0x123", "http://oracle"))
  ...
  it("prevents one account from claiming multiple oracle slots", async function () {
    ...
    await contract.connect(oracle).registerOracle(0n, "did:ethr:sepolia:0x123", "http://oracle-zero");

    await expect(contract.connect(oracle).registerOracle(1n, "did:ethr:sepolia:0x123", "http://oracle-one"))
  ...
  it("prevents another account from replacing an occupied oracle slot", async function () {
    ...
    await expect(contract.connect(otherOracle).registerOracle(0n, "did:ethr:sepolia:0x456", "http://oracle-zero-other"))
  ...
  it("gates requireRegisteredOracleAccount to registered accounts only", async function () {
    ...
    await contract.connect(oracle).registerOracle(3n, "did:ethr:sepolia:0x456", "http://oracle");
