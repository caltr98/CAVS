...
def create_esco_mapper(esco_hierarchy, knowledge_skills):
    """
    Create an ESCO mapper dictionary mapping concept IDs to hierarchy levels.

    Parameters:
        esco_hierarchy (pd.DataFrame): DataFrame containing ESCO hierarchy data.
        knowledge_skills (pd.DataFrame): DataFrame containing knowledge skill data.

    Returns:
        dict: Dictionary mapping ESCO concept IDs to their corresponding hierarchy levels.
    """
    # Initialize the mapper dictionary
    esco_mapper = defaultdict(list)

    # Process ESCO hierarchy data
    for _, row in esco_hierarchy.iterrows():
        for level in range(1, 4):  # Levels 1, 2, and 3
            code = row[f"Level {level} code"]
            term = row[f"Level {level} preferred term"]
            if pd.notnull(code):
                esco_mapper[code].append((level, term))

    # Process knowledge skill data
    for _, row in knowledge_skills.iterrows():
        concept_uri = row["conceptUri"]
        concept_code = concept_uri.split("/")[-1]
        esco_mapper[concept_code].append(("knowledge", row["preferredLabel"]))

    # Convert lists to tuples to make the hierarchy levels immutable
    esco_mapper = {concept_code: tuple(levels) for concept_code, levels in esco_mapper.items()}
...
if __name__ == "__main__":
    ...
    skills_file_name = "./esco/skills_en.csv"
    hierarchy_file_name = "./esco/skillsHierarchy_en.csv"
    skill_file_name = "./esco/broaderRelationsSkillPillar_en.csv"
    transskill_file_name = (
        "./esco/transversalSkillsCollection_en.csv"
    )
    skill_groups_file_name = "./esco/skillGroups_en.csv"
    ...
    esco_skills = pd.read_csv(skills_file_name)
    esco_hierarchy = pd.read_csv(hierarchy_file_name)
    skills_concept_mapper = pd.read_csv(skill_file_name)
    trans_skills_concept_mapper = pd.read_csv(transskill_file_name)
    knowledge_skills = pd.read_csv(skill_groups_file_name)
    ...
    # Write the esco_mapper to a JSON file
    with open(output_mapper_file_name, 'w') as json_file:
        json.dump(esco_mapper, json_file)
    ...
    esco_data.to_csv(output_file_name, index=False)
